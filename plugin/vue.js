...
(function (global, factory) {
  ...
  function isValidArrayIndex (val) {
    ...
    // 若此浮点型大于等于0  且向下取整 和 当前浮点数相等  并且 是一个有效数 则为真
  ...
  function toString (val) {
    return val == null // 判断是否为null
      ? ''             // 如果为null 则返回空字符串
      : Array.isArray(val) || (isPlainObject(val) && val.toString === _toString) // 判断是否是数组或者(json)对象
        ? JSON.stringify(val, null, 2) // 如果是数组或对象 则使用JSON.stringify转换为字符串
        : String(val) // 否则直接使用js的String将val转换为原始字符串
  }
  ...
  function toNumber (val) {
    var n = parseFloat(val); // 使用parseFloat转换为浮点数(若是整数则不带浮点)
    return isNaN(n) ? val : n // 如果转换成功,则返回转换后的数据,否则返回原始数据
  }

  /**
   * Make a map and return a function for checking if a key
   * is in that map.
   * 创建一个map 并且返回一个为检测该map中是包含了key的函数
   * 解释: 检测是否含有以逗号为分隔点的子字符串
   */
  function makeMap (
    str,              // 字符串
    expectsLowerCase  // 是否全是小写
  ) {
    var map = Object.create(null); // 创建一个映射表
    var list = str.split(',');     // 将字符串 以逗号分隔
    for (var i = 0; i < list.length; i++) {   // 将分隔出来的字符串设为真
      map[list[i]] = true;
    }
    return expectsLowerCase  // 是否需要小写
      ? function (val) { return map[val.toLowerCase()]; } // 返回函数 用于检测全小写的val是否存在于映射表中
      : function (val) { return map[val]; }               // 返回函数 用于检测该val是否存在于映射表中
  }

  /**
   * Check if a tag is a built-in tag.
   * 检测是否是内置标签  slot,component
   */
  var isBuiltInTag = makeMap('slot,component', true);

  /**
   * Check if an attribute is a reserved attribute.
   * 检测是否有保留属性: key,ref,slot ,slot-scope,is
   */
   ...
   * 从数组中移除一项
   * @param: arr: 数组  item: 数组项
   */
  function remove (arr, item) {
    if (arr.length) {
      var index = arr.indexOf(item); // 查找数组中是否存在该条数据(没有返回-1,有则返回其下标)
      if (index > -1) {              // 如果有 则删除该项
   ...
   * 检测对象中是否有该属性
   *  --- 暂时不是很理解这里这么做的意义是什么
   *  --- 直接使用obj.hasOwnProperty(key)不好吗?
   *  --- 如果是觉得函数式太长了 在return处使用不一样吗?
   */
  var hasOwnProperty = Object.prototype.hasOwnProperty;
  function hasOwn (obj, key) { // 对象  key
    // 使用call将hasOwnProperty的this指向obj, 然后传入参数key
   ...
   * 创建一个纯函数的缓存版本
   * 这是一个闭包:
   *  在cached函数中 声明一个json变量 cache
   *  然后返回一个闭包函数: 在闭包函数中使用hit来获取key为str的值
   *  如果有则直接返回hit,否则将key为str的数据复制给cache
   *  由于这是一个闭包  所以在调用的时候都会使用到闭包函数
   *  而闭包函数中会使用上层的cache方法
   *  所以cache不会被系统回收
   *  所以实现了缓存机制
   */
   ...
   * 为不支持他的环境简单的绑定polyfill
   *    : polyfill是一个工具,用于兼容不同浏览器对js的实现差异
   * 例如: PhantomJS 1.x.
   *   在技术上,我们不再需要这个,因为原生绑定现在在大多数浏览器中有足够的性能
   *   但是移除它将意味着破坏代码在PhantomJS中运行的能力
   *   所以必须为向后兼容而保持
   *
   * 好---又是一个闭包
   */
  ...
  // 判断是否可以使用funciton的bind函数
  // 如果不能使用则使用polyfillBind
  // 也就是使用apply,call等方式替代
  ...
  function toObject (arr) {
    ...
    for (var i = 0; i < arr.length; i++) {
      if (arr[i]) {
        extend(res, arr[i]); // 调用上面的extend对象进行混合
      }
    }
    return res
  }
   ...
   *  两者是否都是对象,
   *    并且是否都是数组
   *      如果是数组检查数组里每一个元素是否大致相等(递归)
   *    如果不是数组 检查是否是时间对象
   *      如果是是时间对象 判断时间戳是否一样
   *    如果不是时间对象也不是数组
   *      那么可以认为是简单的js对象,那么检查对象的每一个k是否相等(同样递归)
   *    在否则直接返回false
   *      catch是上面代码出现错误 直接返回false
   *  如果两则不是对象
   *    则转换为字符串,看两则的字符串是否相同
   *  否则直接返回false
   */
   ...
   * 返回在数组中查找一个大致相同值的第一个索引,
   * (如果值是一个普通的对象,该数组必须是包含相同模型的对象)
   * 或者 它不存在 则返回 -1
   */
